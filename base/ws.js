...
function connectChannel(channel) {
    if (
        window.conn != null &&
        window.conn.readyState === WebSocket.OPEN &&
...
function unsubscribe(subscriber_id) {
    for (let channel in subscriptions) {
        subscriptions[channel] = subscriptions[channel].filter(
            subscription => subscription.subscriber_id !== subscriber_id
        )
        if (
            window.conn &&
            subscriptions[channel] &&
...
function connectWs(onOpen) {
    if (window.WebSocket) {
        ...
        window.conn.onclose = function () {
            setTimeout(function () { connectWs(onOpen) }, 5000)
        }

        window.conn.onmessage = function (evt) {
            let message = JSON.parse(evt.data)
            if (!subscriptions || !subscriptions[message.channel]) return
