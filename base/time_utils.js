function formatTimeAgo(seconds, lang = "en") {
    let timeStr = formatTimeDiffStr(seconds, lang)
    lang = lang.toLowerCase()
    if (lang === "ru") return (timeStr + " назад").toLowerCase()
    if (lang === "zh") return (timeStr + "前").toLowerCase()
    if (lang === "hi") return (timeStr + " पहले").toLowerCase()
    if (lang === "es") return ("hace " + timeStr).toLowerCase()
    if (lang === "fr") return ("il y a " + timeStr).toLowerCase()
    if (lang === "ar") return ("منذ " + timeStr).toLowerCase()
    if (lang === "bn") return (timeStr + " আগে").toLowerCase()
    if (lang === "pt") return (timeStr + " atrás").toLowerCase()
    if (lang === "id") return (timeStr + " yang lalu").toLowerCase()
    if (lang === "ur") return (timeStr + " پہلے").toLowerCase()
    if (lang === "de") return ("vor " + timeStr).toLowerCase()
    if (lang === "ja") return (timeStr + "前").toLowerCase()
    if (lang === "sw") return (timeStr + " iliyopita").toLowerCase()
    if (lang === "mr") return (timeStr + " पूर्वी").toLowerCase()
    if (lang === "te") return (timeStr + " ముందు").toLowerCase()
    if (lang === "tr") return (timeStr + " önce").toLowerCase()
    if (lang === "ta") return (timeStr + " முன்பு").toLowerCase()
    if (lang === "vi") return (timeStr + " trước").toLowerCase()
    if (lang === "ko") return (timeStr + " 전").toLowerCase()
    if (lang === "fa") return ("قبل " + timeStr).toLowerCase()
    if (lang === "it") return ("fa " + timeStr).toLowerCase()
    if (lang === "th") return (timeStr + " ที่ผ่านมา").toLowerCase()
    if (lang === "gu") return (timeStr + " માટે").toLowerCase()
    if (lang === "kn") return (timeStr + " ಮೂಲಕ").toLowerCase()
    if (lang === "pa") return (timeStr + " ਪਹਿਲਾਂ").toLowerCase()
    if (lang === "ms") return (timeStr + " yang lalu").toLowerCase()
    if (lang === "or") return (timeStr + " ପୂର୍ବେ").toLowerCase()
    if (lang === "my") return (timeStr + " အရင်").toLowerCase()
    return (timeStr + " ago").toLowerCase()
}

function formatTime(seconds, lang = "en") {
    let diff = Math.abs(seconds < 1000000000 ? seconds : seconds - new Date().getTime() / 1000)
    lang = lang.toLowerCase()
